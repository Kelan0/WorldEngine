...
//            float transition = sin(time)
            float hist_x = uv.x;
            float hist_x_norm = getHistogramBinFromLuminance(uv.x * 20.0, histogramHeader.minLogLum, histogramHeader.logLumRange, histogramHeader.binCount);
...
//            if (hist.b >= uv.y) finalColour.b = 1.0;
            ...
            float hist_x_avg = getHistogramBinFromLuminance(histogramHeader.averageLuminance, histogramHeader.minLogLum, 1.0 / histogramHeader.logLumRange, histogramHeader.binCount);
//            hist_x_avg = floor(hist_x_avg * float(histogramHeader.binCount - 1)) / float(histogramHeader.binCount - 1);
            uint hist_ux_avg = uint(hist_x_avg);
...
//        finalColour = finalColour + bloomColour * bloomIntensity;
    ...
    finalColour = vec3(1.0) - exp(-finalColour * histogramHeader.exposure);
//    finalColour = finalColour / (finalColour + vec3(1.0));
    finalColour = pow(finalColour, vec3(1.0 / gamma));
