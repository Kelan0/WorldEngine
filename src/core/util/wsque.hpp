...
// WORK STEALING QUEUE IMPLEMENTATION FROM: https://github.com/taskflow/work-stealing-queue
// Modifications:
// - When resizing, old array is deleted rather than being kept.
...
// Function: empty
template <typename T>
bool wsque<T>::empty() const noexcept {
...
// Function: size
template <typename T>
size_t wsque<T>::size() const noexcept {
  int64_t b = _bottom.load(std::memory_order_relaxed);
  int64_t t = _top.load(std::memory_order_relaxed);
...
// Function: capacity
template <typename T>
int64_t wsque<T>::capacity() const noexcept {
